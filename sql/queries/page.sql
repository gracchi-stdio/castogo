...
SELECT * FROM pages WHERE parent_id = $1 ORDER BY sort_order ASC;

-- name: GetDescendants :many
SELECT * FROM pages WHERE path LIKE sqlc.arg('prefix') || '/%' ORDER BY path ASC;
...
UPDATE pages
SET path = REPLACE(path, sqlc.arg('old_prefix') || '/', sqlc.arg('new_prefix') || '/'),
    updated_at = NOW()
...
SELECT p.path, (SELECT COUNT(*) FROM pages WHERE parent_id = p.id) AS children_count FROM pages p WHERE p.id = $1;
